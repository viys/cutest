...
- `./test.ps1 cmake`：在 `build/test/` 下生成标准版测试工程。
- `./test.ps1 make`：编译 `build/test/` 中的 `cutest` 标准版测试可执行文件；若目录不存在会先自动执行配置。
- `./test.ps1 run`：执行 `build/test/` 中的 `cutest` 测试程序；若可执行文件不存在则提示先执行 `make`。
- `./test.ps1 clean`：清理 `build/test/` 与其子目录中的编译产物但保留 CMake 配置。
- `./test.ps1 delete`：删除仓库根目录下的 `build/`，其中包含标准版、middleware 版和覆盖率构建目录。
- `ctest --test-dir build/test --output-on-failure`：运行标准版构建目录中的已注册测试；middleware 版可使用 `ctest --test-dir build/test/middleware --output-on-failure`。
- `./test.ps1 coverage`：在 `build/test/coverage/` 下启用 GCC/Clang 覆盖率构建并生成 HTML 报告。
